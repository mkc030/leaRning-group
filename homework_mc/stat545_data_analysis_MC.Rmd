...
#Or, just how to look at the number of distinct levels of a variable
n_distinct(gapminder$continent)

#OR, how to see the names of the levels but without the frequency count 
  #note this has to be a factor, does not work on character class!!!
...
Trying to re-order a whole data frame based on values of one column? Try `arrange` from dplyr or else `order` from base R. 
```{r}
#TODO: this won't knit or run as it isn't referring to any actual data; re-write this later
#Example of arranging by date_col using dplyr
df <- arrange(df, date_col)     #arrange in ascending order
df <- desc(df, date_col)        #arrange in descending order

#Examples from base R
df[order(df$date_col), ]        #arrange in ascending order
df[rev(order(df$date_col)), ]   #arrange in descending order
...
* `rex` R package, as it uses more human friendly languate to make regular expressions. Use `names(shortcuts)` to see the full list of available shortcuts and then use inside of `rex()` to generate the equivalent regular expression. I can't find a great easy example of how to use it, but there are a couple of more complicated vignettes that might help if looking into this wrt [webserver log parsing](https://rpubs.com/jimhester/rex-log_parsing) and [url validation](https://rpubs.com/jimhester/rex-url_parsing).
...
* `separate` is similar to `str_split` family but works when variable is in a dataframe. Also, very useful in that it will let you enter missing values into columns if there are unequal number of pieces. See also examples at the end of `?seperate` for other options on how to deal with rows that don't split into the same number of pieces.
...
# To split based on character location in string (note this will replace the original column)
library(readxl)
suppl <- read_excel("/Users/Michelle 1/Documents/R projects/Phase1/Supplementary_Material_ Table S1.xlsx")

splitID_df <- separate(suppl, 
               col = `Sample ID`,
               into = c("locationID", "siteID"))
               
# To split on location but only return ONE of the split columns (note this will replace the original column)
splitID_df <- separate(suppl, 
               col = `Sample ID`,
               into = c("locationID", NA), #as I don't care about the specific site ID
               sep = -2)  #the last two number/letter combo are the site ID
```

* `str_length` counts the length of each character string within the vector

* `str_sub` lets you snip out substrings based on character position. The start and end arguments are vectorized, meaning they can provide a sliding window (for ex, a sliding 3-character window when using a vector rather than just an integer). It can also be used for replacing substrings when placed on the lefthand side of the assignment operator. This has the potential to be a **very powerful** function I think; see `?str_sub` to look at examples at bottom of help file. Very useful for how to potentially split/save sub-strings from vectors or data frames. 
...
# To get a new column with a substring based on character location
  #This example is removing the LAST two characters at the end of the string
  #With character strings that are variable in length
table(str_length(suppl$`Sample ID`)) #Have different no. of characters per sampleIDs

suppl <- suppl %>% 
          mutate(locationID = str_sub(suppl$`Sample ID`, 
                                      start = 1, 
                                      end = nchar(t)-2)) %>%  #note the use of 'nchar' here!!!
...
head(str_split_fixed(fruit, pattern = " ", n = 2), n = 10) 
    #If don't use 'head', this will return 80 lines!
